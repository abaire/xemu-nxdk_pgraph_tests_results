...
extract_xiso_working_path=/root/.local/share/nxdk-pgraph-test-repacker/extract-xiso
readonly extract_xiso_working_path
extract_xiso_cache_path=/work/cache/extract-xiso
...
function download_latest_iso() {
  ...
  iso="/work/cache/nxdk_pgraph_tests-${tag}.iso"
...
function download_latest_xemu() {
  ...
  xemu="/work/cache/$(basename "${app_image_url}")"
...
function download_xbox_hdd_image() {
  hdd="/work/cache/xbox_hdd.qcow2.zip"
  ...
  local hdd_archive
  hdd_archive="/work/cache/$(basename "${artifact_url}")"

  download_artifact "${hdd_archive}" "${artifact_url}"

  unzip -o -q "${hdd_archive}" -d "/work/cache/"
}
...
function main() {
  ...
  if [[ -e "/work/xemu.toml" ]]; then
    cp "/work/xemu.toml" "$(dirname "${xemu}")/"
  fi
  ...
  set +e
  /venv/bin/python3 -m nxdk_pgraph_test_runner \
    --emulator-commandline "${xemu} -dvd_path \"{ISO}\"" \
