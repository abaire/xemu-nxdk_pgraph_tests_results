...
class ResultsInfo(NamedTuple):
    ...
    def parse(cls, run_identifier: str, toc_md_file: str) -> ResultsInfo:
        ...
        return cls(
            ...
            platform_info=components[1],
            gl_info=f"{components[2]}:{components[3]}",
            toc_md_file=toc_md_file,
        )


def generate_markdown_for_test_suite(results_dir: str) -> str | None:
        ...
        for xemu_version in sorted(xemu_version_to_results.keys()):
            ...
            for platform in sorted(by_platform.keys()):
                ...
                for gl_info in sorted(by_gl_info.keys()):
                    ...
                    output_file.write(
                        f"[[{gl_info}|{by_gl_info[gl_info][0].toc_md_file}]]\n"
                    )
    ...
    for root, dirnames, filenames in os.walk(args.results_dir):
        ...
        if filenames:
            ...
            with open(output_filename, "w") as output_file:
                content = generate_markdown_for_test_suite(root)
        ...
        with open(output_filename, "w") as output_file:
            ...
            output_file.writelines(
                [
                    ...
                    "===\n",
                    *[
                        f"- [[{friendly_page_name(result)}|{result}]]\n"
